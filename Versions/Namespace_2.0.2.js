 ...
 *
 * 简化相对目录保存实现过程
 * 增加Component('Components/Button')方法
 */
...
(function () {
    ...
    Namespace.Load = function NS_Load(file) {
        mClassName = file.replace(/.*\//, '');
        var src    = TrimPath(mRootFiles[0], file, '.com');
        if (mCachedClass.has(src)) {
            console.log(`%c 重用：${src} `, mLogStyles.cache);
            return mCachedClass.get(src);
        } else {
            ...
            if (ajax.readyState === 4) {
                if (ajax.status === 200) {
                    try {
                        ...
                        new Function('$$', cmd)({CacheClass, template});
                    }
    ...
    console.groupEnd();
    console.log(`%c开始运行 ${TrimPath(mRootFiles[0], mStart, '.js')}`, mLogStyles.start);
    new Start();
})();
